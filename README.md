...
| **Microsoft 365** (server-side) | live mailboxes via Microsoft Graph (app-only or per-user device sign-in, read-only) | `internal/graph` + [`x/oauth2`](https://pkg.go.dev/golang.org/x/oauth2) |
...
| **New Outlook / Outlook for Mac** | register an app ([docs/graph-app-setup.md](docs/graph-app-setup.md)) | `mailarchive graph …` (`-auth app` or `-auth device`) | `mailarchive schedule -install -- graph …` | No local files exist; only the server-side path works. |
...
| **Microsoft 365, your OWN mailbox (no admin)** | register a public client with delegated `Mail.Read` ([docs/graph-app-setup.md](docs/graph-app-setup.md)); no secret | `mailarchive graph -auth device -out ./archive -tenant T -client-id ID` (sign in once in a browser) | prime the sign-in as the scheduling user, then `mailarchive schedule -install -- graph -auth device -out ./archive -tenant T -client-id ID -token-cache ~/.config/mailarchive/graph.token` | No tenant-wide permission — reads only your own mailbox. First run is interactive; the saved sign-in drives later runs and can expire (re-sign-in when `status` says so). |
