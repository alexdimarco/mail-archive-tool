...
- Upgrading a **pre-existing** live (Graph) archive to format v4 fills in the timeline but does **not** retroactively consolidate its per-folder copies under the new per-mailbox key: its next `graph` run re-downloads and re-stores each still-present message once under the new key (no file is ever deleted, so nothing is lost — the archive just gains a parallel copy). One-copy-per-message applies to a live archive created with this version; to get it on an existing live archive, capture fresh into a new `-out`.
