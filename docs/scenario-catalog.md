...
| MA-244 | U | The #8 closure core (HC1, rev-6.1): two DISTINCT messages that reuse one Message-ID with an IDENTICAL envelope (colliding fingerprints) but different bodies and immutable ids are told apart by the byte compare and BOTH survive; the second is filed under a key qualified by a HASH OF THE IMMUTABLE ID, never the fingerprint (which would collide and overwrite — an R1 drop) | R1, R3, S38, S39 |
| MA-245 | U | phys-churn (HC7, rev-6.1): the SAME message re-observed with a REISSUED immutable id (a restore/migration) is recognized by its BYTE-IDENTICAL archived content, adopted in place (one record, no split/duplicate), its stored PhysID updated to the new id, and a phys-churn anomaly logged; the bytes prove it, so a distinct reuse (MA-244) is never swept up with a churn | R1, R17, S38 |
| MA-246 | U | The live fast-path decides skip-vs-download by the immutable id (rev-6.1 §5): SKIP when a same-token sibling stores THIS id, or backfill a LONE not-yet-identified sibling's id FROM THE LISTING (storm-free upgrade — no re-download; option D's promise), else DOWNLOAD for the exporter to place (adopt/split); a withheld id degrades to the Message-ID-membership floor; with an id, gone-detection stamps only the matched sibling (no EC5 fan-out) so a departed distinct reuse is correctly detected | R17, S38, S39 |
| MA-247 | U | HC2 (collapse × PhysID, rev-6.1 §6): a move-duplicate the v3→v5 collapse unified into ONE LiveKey survivor (loser file in AlsoFiles for redaction reach, R13) has its PhysID backfilled FROM THE LISTING on the first immutable-id-honoring walk — no re-download, AlsoFiles preserved — so no new merge mechanism is needed; a still-live distinct sharer re-separates via the ordinary distinct-id split (MA-203) | R13, R17, S38, S39 |
...
| MA-203 | U | a DISTINCT message reusing an already-archived Internet-Message-ID is split into its own qualified mailbox-wide sibling so both survive (R1, MA-86). On a DOWNLOAD path (local import, full run) the split is keyed by content fingerprint; on the live Graph path the distinct reuse is told apart by its Graph IMMUTABLE ID (distinct per physical message, EC6) — the fast-path finds no sibling with that id, downloads it, and the exporter's byte compare #fp-splits it under a PhysID-HASH key (identical envelopes collide under an fp key — HC1), while the unchanged original is skipped by its matched id. The former #8 Graph FLOOR residual is now CLOSED on Graph (closure rev-6.1); IMAP/local without a per-message id remain the logged floor | R1, R3, R17, S38 |
