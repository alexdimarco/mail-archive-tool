...
| MA-218 | U | the first live (Graph) run over a PRE-EXISTING v3 archive holding a per-folder move-duplicate WIRES CollapseByIdentity into RunGraph: the two folder-scoped records collapse to one mailbox-wide LiveKey record BEFORE capture, so the message is recognised by identity and its body is NOT re-downloaded (R17), the archive ends with one record per message (R3), the loser file is kept on disk (R13), and a distinct-fingerprint reuser would stay a #fp sibling (R1, MA-198); idempotent if a run does not persist | R17, R2, R1, S39 |
