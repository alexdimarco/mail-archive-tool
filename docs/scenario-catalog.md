...
| MA-79 | L | on a real Windows box: `schedule -install` writes the wrapper and the task, the task runs the wrapper at the set time, the job logs to <out>/<name>.log and crashes to .stderr.log, `-remove` deletes all three — **pending**, validated by hand per README → Windows notes | R14, S28 |
