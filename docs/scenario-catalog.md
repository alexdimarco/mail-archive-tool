...
- **R17 — Graph capture is complete, read-only, incremental.** The Microsoft
  Graph source — **app-only OR delegated per-user (`-auth app|device`)** —
  enumerates every folder (including nested) — excluding the
  ...
  only GET requests (**app-only holds the read-only `Mail.Read` application
  permission and addresses `/users/{upn}`; delegated holds delegated read-only
  `Mail.Read` for the signed-in user's OWN mailbox and addresses `/me`** — either
  way a mailbox is never modified); and an incremental re-run archives
  zero new items and re-downloads no already-archived message body, whichever mode
  captured it (the store key is seeded from the mailbox id, not the auth mode). A cross-folder
  Internet-Message-ID hit costs only a Message-ID membership check (no body
  fetched) and downloads only a Message-ID not yet archived anywhere in the
  mailbox; an already-archived message that merely moved between folders is
  recorded as a folder change with no download (a distinct message that REUSES an
  archived id is the bounded #8 residual on this floor build — skipped without
  capture, closed by the deferred Graph ImmutableId closure).
...
| S20 Server-side archive of a (large) M365 mailbox via an app-only Graph grant, or a per-user delegated (device-code) grant against the operator's OWN mailbox | R17, R2, R9 | full first capture; incremental re-run adds nothing and re-downloads nothing; read-only; delegated mode signs in once by device code, addresses `/me`, caches a refresh token for later silent runs, archives only the signer's mailbox, and continues the SAME archive an app-only run started | MA-61, MA-62, MA-63, MA-64, MA-251, MA-252, MA-253, MA-254, MA-255, MA-256, MA-257, MA-258, MA-259, MA-260, MA-261 |
...
| S39 A v3 archive keyed by the pre-v4 folder-scoped format is opened by the live path, or two messages reuse one Message-ID across folders | R1, R2, R3, R5 | records sharing a (token, identity) with an equal, non-empty fingerprint collapse to one mailbox-wide LiveKey record (first-captured file kept, R13; each loser's kept-on-disk file recorded in AlsoFiles so a later redaction reaches every copy — collapse fabricates no history event, its pre-upgrade folder history is genuinely unknown); the collapse discriminates key shape by identity position (not NUL count) so an organic #fp-qualified LiveKey is left verbatim, and is scoped+re-keys the index atomically; distinct-fingerprint reuses stay #fp-qualified siblings (no silent drop); a pre-v6 collapsed survivor (no content hash) stays the Message-ID floor on an immutable-id-honoring walk — skipped by membership, no re-download, PhysID left unset, its AlsoFiles (redaction reach) preserved (HC2, rev-6.2); the v2→v3 re-scope is version-gated so a folder-less one-NUL live key is never mis-read as v2; the append-only history log round-trips and tolerates a torn tail (write-truncate + read-skip); Load refuses a stored version above 6 | MA-198, MA-199, MA-200, MA-201, MA-224, MA-225, MA-226, MA-240, MA-247, MA-249 |
| S38 A live-source (Graph) message moves between folders across runs, or a distinct message reuses an already-archived Message-ID in another folder | R3, R17, R1, R2, R5 | one physical copy is kept at its first-captured folder (R13); an incremental re-run records the move as a manifest folder change (Folder/LastSeen/Present) + a history folder-assertion + a body-free index folder update, with no re-download; a distinct id-reuse is split into a qualified mailbox-wide sibling so both survive (R1): on a DOWNLOAD path by content fingerprint, and on the live Graph path by the Graph IMMUTABLE ID + a recorded CONTENT HASH (a reuse whose id matches no v6 sibling is downloaded and the exporter's content-hash arbiter #fp-splits it under a PhysID-hash key — #8 CLOSED on Graph for fresh-v6 records, EC6), while a SECOND content-equal id (a reissued id, or a copy of the message in another folder) is ADDED to the record's content-equal set — one record, no duplicate, and every copy skips on later runs with no re-download or anomaly (rev-6.2 id-set); an id-withheld tenant, or a pre-v6/floor record with no content hash, stays the Message-ID-membership floor (no re-download storm, no unsafe id backfill); a full run re-materialises no per-folder duplicate; the crash order (history append+fsync → index → manifest anchor) never advances the manifest past the events that explain a move; a no-Message-ID message deduped on its content hash still stamps LastSeen and follows a move at the exporter's skip (re-downloaded each run, the acknowledged no-mid price) | MA-202, MA-203, MA-204, MA-205, MA-206, MA-244, MA-245, MA-246, MA-247, MA-248, MA-249, MA-250 |
...
incremental fast-path recognises an already-archived message by Message-ID
MEMBERSHIP — no envelope signature — so an archived id is skipped without download
whatever folder it is now in (R17), and a distinct message reusing that id is the
bounded #8 residual on this floor build (skipped without capture, closed by the
deferred Graph ImmutableId closure; a DOWNLOAD path — local import or full run —
splits it by content fingerprint, R1), but a message with NO
...
| MA-251 | U | a delegated (device-mode) Graph client addresses `/me` for folders, child folders, per-folder messages, the well-known-folder resolution and the `/$value` MIME fetch (never `/users/{upn}`), walks the full tree and pages all messages, and issues ONLY GET requests to Graph (read-only); an app-mode client on the same fixture still addresses `/users/{upn}` | R17, S20 |
| MA-252 | U | the device path sends `Prefer: IdType="ImmutableId"` on the listing and the `/$value` fetch exactly as app-only, and sets `MessageRef.PhysID` only when the tenant honors it (Preference-Applied), so the whole rev-6.2 id machinery is mode-independent | R17, S38 |
| MA-253 | U | device mode reads the signer's `userPrincipalName` from `GET /me` (not a self-asserted id_token) and archives that one mailbox: an omitted `-mailbox` adopts the signer; a `-mailbox` equal to the signer (case-insensitive) is accepted; a `-mailbox` naming anyone else is refused with a typed non-zero before any walk, naming the signed-in user | R17, R12, S20 |
| MA-254 | U | a first device run with no cached sign-in performs the device-code grant against the test-overridable device-auth/token endpoints (prints the verification URI + user code, polls `DeviceAccessToken`), then archives; a device run whose token or Graph endpoint stalls before headers fails within the configured deadline (never hangs, R17/S29) | R17, S20, S29 |
| MA-255 | U | the token cache is read through the shared `util.ReadSecureFile` discipline (regular file only — symlink/FIFO/device refused; Unix `0o077`-clear else a `chmod 600` refusal; a 64 KB bound) and is written atomically `0600` bound to one signer (the stored UPN round-trips), under the OS config dir (never inside `-out`); the `-mailbox`-vs-signer refusal is MA-253 | R17, R12, R4, S20 |
| MA-256 | U | an UNATTENDED device run with no usable token cache is refused with a typed non-zero naming the interactive-sign-in remedy (a scheduled run cannot prompt); a run with a primed cache runs silently (the positive twin) with no prompt; `status` maps an `invalid_grant`/expired-refresh last-run error to a re-consent remedy line | R17, R12, R18, S20 |
| MA-257 | U | `-auth device` with `-client-secret-file` or an explicit `-client-secret-env` is refused (a public client takes no secret, so a secret is never fed to a flow that ignores it); `-auth` with any value other than `app`/`device` is refused naming the two; each refusal is a typed non-zero and leaks no secret | R17, R12, S20 |
| MA-258 | U | token write-back is race-safe and atomic: a refreshed token (later expiry) is persisted via temp-file + fsync + rename and round-trips; a token no newer than what is on disk does not overwrite it (a concurrent rotation is not clobbered); a failed/torn write never replaces a valid cache | R17, R5, S20 |
| MA-259 | U | an archive first captured `-auth app` and then continued `-auth device` for the SAME mailbox (or vice-versa) exports zero new items and re-downloads no body on the second run — the store token is seeded from the mailbox id, not the auth mode, so the incremental fast-path holds across a mode switch | R17, R2, R3, S20 |
| MA-260 | U | `schedule` accepts a `graph -auth device -token-cache PATH` job and validates the cache at install time (present, secure per MA-255, parses, carries a refresh token) — an unprimed/missing cache is refused naming the sign-in remedy; a device job carries `-auth device -token-cache PATH` (never a secret file) into the canonical job and the preview shows it; an app job is unchanged | R14, R12, S28, S20 |
| MA-261 | L | end-to-end device-code consent + delegated `Mail.Read` walk against a real M365 mailbox (the consent screen names `mailarchive`; a user-consent-disabled tenant falls back to one-time admin consent) — **pending**, validated on a live tenant | R17, S20 |
...
| MA-218 | U | FoldEvents INVALIDATES a run whose header timestamp does not parse (a valid-JSON but impossible date, from corruption or a hostile writer): its per-message events do NOT inherit the previous run header's date, so a bad header can neither resurrect a message that was gone as of D nor hide one that was present — matching RunDates, which already skips such a header for the date track | R21, S37 |
| MA-219 | U | the mailbox-wide exporter skip path emits a folder-assertion on PRESENT-AGAIN (pre-observation record not Present) as well as on a MOVE, even when the folder is UNCHANGED, so a no-Internet-Message-ID message (deduped post-download on its content hash, so it takes this skip path not the pre-download fast-path) that went gone and reappeared in the same folder is folded present again, not gone forever after | R5, S37, S38 |
| MA-220 | U | `reindex` saves the reconciled manifest (the keep-authority for history compaction) BEFORE compacting the log, so a compaction failure or a crash between the two still persists the redaction and the derived log heals from (manifest ∩ disk) next run — the manifest never leads the log (§3.5 for the redaction path) | R21, R5, S37 |
| MA-221 | U | Manifest.SetPresent flips ONLY a record's Present flag (leaving Folder/LastSeen; false for a missing key) — the exact undo of a SweepGone gone-flip the live path applies when the paired {k,gone} history append fails, so the saved manifest never durably records a gone the log lacks and the message is re-detected gone next run | R5, S37 |
...
| MA-201 | U | v4 key-format discrimination is version-gated: a folder-less one-NUL live key in a v4 manifest is not re-scoped (Rekeyed==0) though the identical bytes are re-scoped under a v2 manifest; Load refuses a stored version above 6 (version 7) naming the file and the upgrade remedy, byte-unchanged | R5, R12, S30, S39 |
| MA-222 | U | manifest format v5: a Record carries FpScheme (the fingerprint-scheme tag — 0/legacy means the stored fp is NOT comparable to a current one, so the download #fp-split adopts-never-splits against it) and AlsoFiles (extra on-disk paths a collapse folded in, which redaction deletes); both round-trip across save+load and a pre-v5 record loads FpScheme=0 with no AlsoFiles | R5, R12, S39 |
| MA-223 | U | adopt-never-split: a DOWNLOAD that lands on a same-identity sibling whose stored fingerprint is LEGACY-scheme (FpScheme==0, not comparable to a current fp) treats it as the SAME message and ADOPTS the record in place with the current-scheme fingerprint — never #fp-splitting a migrated record into a duplicate (R3) — while a genuine current-scheme mismatch still splits (R1) | R1, R3, S39 |
| MA-224 | U | the v3→v5 collapse discriminates key SHAPE by which component is the message identity (mid:/sha:), never by NUL/component count: a healthy #fp-qualified LiveKey (token\x00identity\x00fp) has three components like a v3 folder-scoped key but is kept VERBATIM (EC4, else a distinct id-reuser is merged/dropped — a regression); and the collapse is SCOPED to the passed token(s), leaving another mailbox not archived this run untouched (R3) until it is itself archived, returning the survivor old→new re-key map | R1, R3, S39 |
| MA-225 | U | idx.Rekey applies the collapse to the search index in one durable (wal_checkpoint+fsync) transaction — renaming each survivor row old→new and deleting each collapsed loser row so the index tracks the manifest re-key, durable BEFORE the manifest advances (EC5) — and is idempotent (a remap source already re-keyed is skipped), so a crash-retry is safe | R8, S39 |
| MA-226 | U | the option-D v3→v5 upgrade end to end (genuine v3 fixture): a v3 Graph archive holding a per-folder move-duplicate (Inbox+Trash, one identity, same legacy fp, real files + v3 index) collapses to ONE mailbox-wide LiveKey record BEFORE the walk and re-keys the index, so the first upgraded run recognises the message by Message-ID membership and does NOT re-download it (R17), keeps one record for the identity at the first-captured file (R3), and the loser copy stays on disk (R13) | R17, R3, R1, S39 |
| MA-227 | U | redaction reaches every copy: a collapsed record carries its move-duplicate loser file in AlsoFiles, and reindex — when the survivor .html is deleted (redacted) — sweeps the survivor OWN orphaned siblings (.eml + -attachments.zip) AND the whole loser triple, so no copy of a redacted message (the sensitive .eml/.zip included) stays served at /files/; this runs ONLY in reindex, never a normal run (R13) | R21, R13, S39 |
| MA-228 | U | the bounded #8-residual id-reuse note fires ONLY on genuine ambiguity (an identity already archived under >1 distinct fingerprint, e.g. a reuse migrated from v3) and is deduped per identity per run, so a real Message-ID reuse is flagged (a further distinct reuse could be skipped without capture on the floor — the deferred ImmutableId closure detects it) without flooding; an ordinary single-sibling skip logs nothing | R1, S39 |
| MA-229 | U | the membership skip advances LastSeen on EVERY same-token same-identity sibling (TouchSeen — folder untouched), so a still-present #fp-qualified sibling of a reused Message-ID is not phantom-marked gone by SweepGone (EC5); a single listing entry cannot say which physical sibling it is, so all are stamped observed (a genuinely departed sibling of a reused id is the deferred-closure case, not detected here) | R21, R1, S38, S39 |
| MA-230 | U | membership dedup is MAILBOX-scoped: KeysForIdentity is archive-wide, so the fast-path filters to the current token before skipping — two mailboxes archived into one -out that share a Message-ID EACH keep their own physical copy (R6), the second is never skipped onto the first (which would drop it and corrupt the first timeline, R1/R21) | R1, R6, S38, S39 |
| MA-231 | U | adopt-never-split applies ONLY when the identity has a single same-store record (the unambiguous single-message migration); with MORE THAN ONE record (a distinct Message-ID reuse survived collapse) the guard is off and the message #fp-splits by content, so a full-mode re-export never adopt-overwrites (drops) a sibling — both survive (R1/R3), covering full mode which bypasses the membership skip | R1, R3, S38, S39 |
| MA-232 | U | reindex redaction gates every record Path and AlsoFiles entry with validRelPath before os.Remove, so a tampered manifest `../`/absolute path cannot delete a file OUTSIDE -out (INS2-1); an unsafe entry is skipped and logged, mirroring Rebuild and the other manifest-path consumers | R13, S30, S39 |
| MA-233 | U | the dedup identity and fingerprint fold the IMMUTABLE MIME Date (IdentityDate), never the mutable delivery time (Received, which the Graph path overwrites with receivedDateTime for display/naming) — so a no-Message-ID message's sha: identity and its fingerprint stay stable across a pre-go-back→v5 upgrade (no drift → no duplicate, no phantom-gone); a source with no MIME Date falls back to Date(), leaving local/PST fingerprints unchanged | R1, R3, S38, S39 |
| MA-234 | U | the v3→v5 collapse tests the identity at parts[2] BEFORE the LiveKey (parts[1]) case, so a v3 folder-scoped key whose FOLDER is literally named mid:…/sha:… is still migrated — not mis-read as an already-collapsed LiveKey and left to re-download+duplicate+phantom-gone on the next live run | R3, R17, S39 |
| MA-235 | U | the collapse merges same-(token,identity) records ONLY when their on-disk files are byte-identical (a real move-duplicate); two DISTINCT messages that reused one Message-ID with an identical envelope (same fingerprint — bodies excluded) but differing files both survive as distinct records (merging would drop one from the manifest+index — R1), and a same-fingerprint key collision is qualified by a per-key hash so neither is lost | R1, R3, S39 |
| MA-236 | U | the collapse move-duplicate discriminator compares the PRESERVED .eml (raw wire bytes, folder-independent), NOT the rendered .html (which embeds the folder breadcrumb, depth-dependent paths and key-derived names and so differs between a message's two folder-copies): a genuine move-duplicate whose renderings differ but whose .eml is identical is merged (no R3 duplicate), distinct .eml keeps both (R1), and no .eml falls back to merge (the common case) | R3, R1, S39 |
| MA-237 | U | the one-time v3→v5 collapse is DEFERRED when a search index exists on disk but is not open this run (-index=false): collapsing would re-key the manifest to LiveKeys while leaving the index at its v3 folder-scoped keys — a permanent desync a later reindex redaction could not reconcile — so the token still owes the collapse (unmarked) and an -index run performs it, re-keying both in step | R3, R8, S39 |
| MA-238 | U | a full-mode Graph run NEVER deletes an already-archived LEGACY-scheme record's file when a same-Message-ID message is re-observed: the legacy record's identity is not content-verified, so the observed message may be a DISTINCT reuse of a gone original — deleting its file would be irreversible R13 data loss. The file is kept (verify flags any orphan); the residual manifest/search record-overwrite in this rare reused-id + full-mode case is resolved by the deferred ImmutableId closure | R13, R1, S39 |
| MA-239 | U | PhysID is a capture-time hint, NEVER part of the message identity: Identity/Fingerprint/contentHash never fold it, so two messages differing only in PhysID hash identically — a reused-id distinct message is told apart by content (not a transport id) and a changed immutable id never drifts a sha: identity | R1, R3, S39 |
| MA-240 | U | manifest format v6 adds Record.PhysID (the per-physical-message immutable-id hint): it round-trips across save+load, and a pre-v6 record loads with an EMPTY PhysID so the live path degrades to the Message-ID floor until a run backfills it; Load refuses a stored version above 6 (version 7) | R5, R12, S39 |
| MA-241 | U | the Graph client requests immutable ids (Prefer: IdType="ImmutableId") on the listing and the /$value fetch, and sets MessageRef.PhysID to the message id ONLY when the tenant HONORS it (Preference-Applied present, decided ONCE per run from the first listing page); a tenant that ignores the preference leaves PhysID empty so the live path degrades to Message-ID membership (the floor), never treating a mutable id as immutable | R17, S38, S39 |
| MA-242 | U | HC5 capture-store: the live-path (mailbox-wide) Exporter records the source's immutable id as Record.PhysID; when a later re-download of the SAME message arrives with the id WITHHELD (empty), the previously-stored PhysID is carried FORWARD, not cleared, so a distinct-reuse split keeps a comparison baseline. Inert — never folded into identity/fingerprint (see MA-239) | R17, S38 |
| MA-243 | U | HC10 + rev-6.2 id-set: the mailbox-wide identity index carries a record's primary PhysID; AddPhysID records content-equal immutable ids (the first becomes the primary and is indexed, further ones append to AltPhysIDs) so ONE record represents several live copies of a message without flipping a single slot; HasPhysID matches any id in the set; no-ops (empty id, unknown key, already-known) report false. Never touches identity or fingerprint | R17, S38 |
| MA-244 | U | The #8 closure core (HC1, rev-6.2): two DISTINCT messages that reuse one Message-ID with an IDENTICAL envelope (colliding fingerprints) but different bodies and immutable ids are told apart by the recorded body-inclusive CONTENT HASH (no .eml/-raw needed) and BOTH survive; the second is filed under a key qualified by a HASH OF THE IMMUTABLE ID, never the fingerprint (which would collide and overwrite — an R1 drop) | R1, R3, S38, S39 |
| MA-245 | U | id-set absorb (rev-6.2): a SECOND content-equal immutable id for one Message-ID — a reissued id (a restore/migration) or a copy filed into another folder — is recognized by its identical CONTENT HASH (excludes transport headers, so a header-rewriting migration still matches) and ADDED to the record's content-equal set (AddPhysID), not treated as a churn that flips a single slot: one record, no split/duplicate, no anomaly, and BOTH ids skip on a later run (no re-download, no id flap). The content hash proves sameness, so a distinct reuse (MA-244) is never swept up with it | R1, R17, S38 |
| MA-246 | U | The live fast-path decides skip-vs-download by the immutable id + the recorded content hash (rev-6.2 §5): a message captured under v6 stores its immutable id AND a content hash, and a re-run SKIPS it by its EXACT id with no re-download; SKIP also when a same-token sibling stores THIS id; DOWNLOAD (for the exporter's content-hash placement) only when the id matches no sibling AND every same-token sibling carries a content hash; a withheld id, or ANY sibling without a content hash (a pre-v6/floor record), stays the Message-ID-membership floor (no re-download storm, no unsafe id backfill); with an id match, gone-detection stamps only that sibling so a departed distinct reuse is correctly detected | R17, S38, S39 |
| MA-247 | U | HC2 (collapse × PhysID, rev-6.2 §6): a move-duplicate the v3→v5 collapse unified into ONE LiveKey survivor is a PRE-v6/floor record with NO content hash (loser file in AlsoFiles for redaction reach, R13); on the first immutable-id-honoring walk the closure does NOT engage against it — it stays the Message-ID floor (skipped by membership, NO re-download, AlsoFiles preserved, PhysID left UNSET, never binding a listed id to a record it cannot content-verify — HC4); the archive closes #8 only for messages captured fresh under v6 | R13, R17, S38, S39 |
| MA-248 | U | the content-hash arbiter (Record.ContentHash / model.ContentDigest) is at least as discriminating as Fingerprint (rev-6.2, adversarial re-check): it folds Subject/Sender/To/Cc/Bcc/immutable-date + each attachment's NAME and SIZE + all body variants, so two DISTINCT reuses of one Message-ID that differ only in an attachment name (same count), Cc, or Bcc get different digests and BOTH survive — a coarse count-only digest would collide them and the closure would adopt-and-drop one (silent R1 loss). It is not the sha: identity's contentHash and never shifts a key | R1, R3, S38 |
| MA-249 | U | rev-6.2 id-set: a message COPIED into two folders (same Internet-Message-ID, identical content, DISTINCT Graph immutable ids) collapses to ONE mailbox-wide record whose content-equal id set holds BOTH ids; the first run downloads each copy once (a fresh capture + one content-compare) and adds both ids, and every LATER run skips both by set membership — no re-download, no id flap, no bogus timeline move from a flip. Without the set a single-id record re-downloads the other copy every run forever | R17, S38, S39 |
| MA-250 | U | rev-6.2 id-set durability: the content-equal immutable-id set survives a record REBUILD (a full-mode re-export or a fill) — the rebuild carries {new primary} ∪ {prior primary} ∪ prior AltPhysIDs forward, so a copied message keeps ALL its ids and is not re-downloaded on the next incremental run; without the carry-forward a full run would strand every copy but the primary | R17, S38 |
| MA-202 | U | an incremental Graph re-run over a message MOVED between folders records the move as a manifest folder change (Folder/LastSeen/Present) + a history folder-assertion event + a body-free `UPDATE docs SET folder`, keeping ONE physical file at its first-captured folder and downloading no body (recognised by Message-ID membership, so no re-fetch); the timeline log carries the run header, the new-capture assertions, and the move | R17, R3, R1, R5, S38 |
| MA-203 | U | a DISTINCT message reusing an already-archived Internet-Message-ID is split into its own qualified mailbox-wide sibling so both survive (R1, MA-86). On a DOWNLOAD path (local import, full run) the split is keyed by content fingerprint; on the live Graph path the distinct reuse is told apart by its Graph IMMUTABLE ID (distinct per physical message, EC6) — the fast-path finds no sibling with that id, downloads it, and the exporter's CONTENT-HASH arbiter #fp-splits it under a PhysID-HASH key (identical envelopes collide under an fp key — HC1), while the unchanged original is skipped by its matched id. The former #8 Graph FLOOR residual is CLOSED on Graph for messages captured fresh under v6 (closure rev-6.2); pre-v6/floor records and IMAP/local without a comparable content hash remain the logged floor | R1, R3, R17, S38 |
| MA-204 | U | Exporter.DedupMailboxWide off keeps the folder-scoped key so a one-shot local import stores the same mail in each folder (R3); on keys mailbox-wide by identity (the CONTENT fingerprint is the distinct-reuse discriminator, option D) so one message is one file across folders and modes, and a full run re-materialises no per-folder duplicate (R2, full still re-exports all) | R3, R2, R17, S38 |
