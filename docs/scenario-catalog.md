...
| MA-203 | U | a DISTINCT message reusing an already-archived Internet-Message-ID is split by CONTENT fingerprint (option D — not an envelope signature) into its own #fp-qualified mailbox-wide sibling whenever it is DOWNLOADED (a local import, a full run, or — once built — the deferred Graph ImmutableId closure), so both survive (R1, MA-86 mailbox-wide). On the incremental Graph FLOOR the ImmutableId closure is deferred, so such a reuse is skipped by Message-ID membership pre-download and NOT archived — the bounded #8 residual the operator accepted for the floor build, xfail-encoded (TestGraphDistinctIDMailboxWideSplit) and closed by that closure | R1, R3, R17, S38 |
