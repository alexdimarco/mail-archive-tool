...
**Verdict: GO_WITH_CONDITIONS.** Conditions S1–S16 are must-fix-before-code and are folded
into design revision 2. Per S1 the work is split into slices that each owe their own gate.
...
|---|---|---|---|---|---|
| F1 (lens 2) | 2 | §3.5, §4 | Seven independent units and a grab-bag of GUI flow fixes bundled into one design | PARTIAL (high) | **S1** |
| F2 (lens 1), F3 (lens 5), L5-5 | 1, 5 | §3.1 | `-- serve` (or search/status) is accepted and installs a never-terminating job | CONFIRMED (high) | **S2** |
| L5-7, F7 (lens 5) | 5 | P1 | The flat form and the `--` form can both carry `-out`/`-mode` with no precedence | CONFIRMED (medium) | **S3** |
| L6-1, F1 (lens 6) | 5, 6 | §3.2 | Batch quoting ignores cmd.exe metacharacters (`&` is legal in NTFS names) | CONFIRMED (high) | **S4** |
| L3-1, F6 (lens 1), F6 (lens 8), F5 (lens 5), F3 (lens 7) | 3, 4, 5, 7, 8 | §3.4 | Nothing links a schedule to its archive; status can only probe the default name; on Windows it stats the wrapper, not the real exe | CONFIRMED (high) | **S5** |
| F4 (lens 7), F1 (lens 10), F5 (lens 1) | 1, 7, 10 | P5, §3.4 | No posture for "no last run"; a killed run leaves the prior "ok"; "every run leaves a record" is false under SIGKILL | CONFIRMED (high) | **S6** |
| F4 (lens 5), F7 (lens 8) | 5, 8 | §3.5 | GUI and CLI both default to one name; a second archive silently overwrites the first's schedule | CONFIRMED (medium) | **S7** |
| L3-2, F3 (lens 9), F5 (lens 8) | 3, 4, 8, 9 | P7, §3.4 | Exe moved/upgraded → job silently dead; no rollback story | CONFIRMED (high/medium) | **S8** |
| L3-3, F2 (lens 9), F8 (lens 7) | 3, 7, 9 | P3, §3.5 | Logs append forever inside the (synced) archive | CONFIRMED (medium) | **S9** |
| L6-4, F8 (lens 6), L3-7 | 3, 6 | §3.1, §3.3 | Secret-file mode check only at 02:00; FIFO/symlink accepted; secret expiry has no story | CONFIRMED/PARTIAL | **S10** |
| L3-6, L3-4 | 3, 4 | §3.5 | Job file lacks `auto` (new stores never picked up) and a version field | CONFIRMED (medium/low) | **S11** |
| F8 (lens 1), L3-5, F8 (lens 10) | 1, 3, 10 | P3, P7 | The GUI's direct `/TR` is unbounded; `-name` is unbounded | CONFIRMED/PARTIAL | **S12** |
| L3-8 | 3 | §3.2 | Install order unstated (task before wrapper leaves a task pointing at nothing) | CONFIRMED (low) | **S13** |
| F4 (lens 9) | 9 | §3.4 | "No crontab" and "no cron at all" collapse to "not installed" | PARTIAL (low) | **S14** |
| L5-8, F6 (lens 5) | 5 | §3.4 | `status` exit code undefined | CONFIRMED (low) | **S15** |
...
| F2 (lens 7) | 7 | §3.5 | Headless job-file failure has no sink independent of `out` | PARTIAL (medium) | **S11** |
| F7 (lens 2) | 2 | §4 | R18 restates R14 | PARTIAL (low) | **S16** |
| F5, F7 (lens 10) | 10 | §6 | Wizard flows and real-host status detection labelled "proven" | PARTIAL (low) | **S16** |
| (2 refuted) | — | — | recorded in the journal with refuting quotes | REFUTED | — |

## Conditions (all folded into revision 2)

- **S1 — Split.** (a) job parsing + `--` grammar + verb whitelist + Windows wrapper; (b)
...
- **S2 — Verb whitelist.** A `--` job may be the default export, `graph`, or `reindex`;
  `serve`/`search`/`status`/`schedule` are refused at schedule time naming why.
- **S3 — One form per invocation.** When `--` is present, any schedule-level export flag
  is refused naming it; `-out`, the log path and all validation derive from the job.
- **S4 — Batch quoting.** The wrapper quotes every token unconditionally and doubles `%`;
  delayed expansion is never enabled; MA-73 covers `& | < > ^ ( ) !`.
- **S5 — Archive-local descriptor.** Install writes `<out>/.mailarchive-schedule.json`
...
- **S6 — Fail-closed posture.** The last-run record is written at START (`running`, pid)
...
- **S7 — One schedule per archive.** The default name is `mailarchive-<8 hex of the
  absolute -out>` on both surfaces; `-name` still overrides; `DefaultName` remains the
  managed-marker prefix.
- **S8 — Exe identity.** The descriptor records the installed exe path + size + mtime;
...
- **S9 — Bounded logs.** A `-log FILE` flag (export and graph) writes the operator log to
...
- **S10 — Secret file.** Schedule-time validation calls the same reader as run time:
...
- **S11 — Job file.** Carries `version` and `auto` (re-discovery at run time); the last-run
...
- **S12 — Length bounds.** `-name` is sanitized and capped at 40 characters; Install
...
- **S13 — Install order.** Wrapper written (and fsynced) before the task is created.
- **S14 — Three-state detection.** `Installed` returns installed / not installed /
  scheduler unavailable, and `status` names an unavailable scheduler.
- **S15 — Exit code.** `status` exits 0 whenever it reports; posture is data on stdout
  (X8). A machine-readable health signal is tracked as ux-contract condition C5.
- **S16 — Honesty.** R18 is narrowed to the new property (every scheduled run is recorded;
