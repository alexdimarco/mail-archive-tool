...
| X3 | **Help is total.** The root and every subcommand emit usage naming their flags. | Met | MA-39 walks `-h`, `serve -h`, `search -h`, `reindex -h`, `schedule -h`, `graph -h` and asserts each names its flags. The root `-h` also enumerates every subcommand (`exportUsage`). |
| X4 | **One grammar.** Subcommands are verbs (`serve`, `search`, `reindex`, `schedule`, `graph`); flags are kebab-case (`-copy-first`, `-enable-offline`, `-sync-wait`, `-client-secret-env`); repeated concepts share a name (`-out`, `-input`, `-mode`). A *flag* (`-outlook`) augments the default export; a *subcommand* (`graph`) is a distinct acquisition mode. | Met (by convention) | Reviewed; consistent. **C4:** a parser-tree lint would make this structural (backlog). |
| X5 | **Flags mean the same everywhere.** `-out` is the archive dir across export/serve/search/reindex/schedule/graph; `-input` is always a source; `-mode` is always incremental/full. | Met | Same flag names carry the same meaning across every surface (`cmd/mailarchive/main.go`). |
