...
|---|---|---|---|
| X1 | **Typed exits, named once.** Exit codes come from one declared set. | Partial | Today: `0` ok — including a handled interrupt (a single Ctrl-C / SIGTERM), which logs "interrupted; progress saved to the manifest" and exits 0 · `1` any error/refusal. The tool never returns `130` itself; only a signal it does not catch reaches Go's default handler, which the shell reports as 128+signum. Refusals are proven to return non-zero and name the problem (MA-33, MA-34). **`verify`** is the one verb with a semantic result in its exit status: `0` attested (every recorded file checked and intact, none unrecorded) · `2` not attested (any modified/missing/unrecorded file, each named; `unexpected` is reported, not an exit condition) · `1` refusal or error (no manifest, locked archive, unreadable manifest) — declared here and covered by MA-135 (exit 0), MA-136 (exit 2) and MA-140 (exit 1). This `2` is verify's own not-attested signal, unrelated to C1's proposed usage/refusal split. **`extract`** declares its own non-colliding set (PC11): `0` the whole requested set emitted (nothing skipped) · `3` **partial** — some records had no preserved bytes to emit; a FRESH code, distinct from verify's `2`, and the emitted==0 case (nothing extractable, e.g. a PST-only archive) maps HERE, never to `0` and never to an error · `1` refusal/error (no manifest, a `-dest` that equals/sits inside/contains `-out`, a non-empty `-dest` without `--overwrite`, ENOSPC mid-write) — covered by MA-181/MA-182 (exit 0), MA-183 (exit 3), MA-184/MA-185 (exit 1). This `3` is extract's own partial signal, unrelated to verify's `2`. **C1:** split usage/refusal to a distinct `2` from runtime `1`; until then `assure.Refused` asserts `Code(1)`. |
...
| X3 | **Help is total.** The root and every subcommand emit usage naming their flags. | Met | MA-39 walks `-h`, `serve -h`, `search -h`, `reindex -h`, `schedule -h`, `graph -h`, `status -h`, `verify -h` and asserts each names its flags. The root `-h` also enumerates every subcommand (`exportUsage`). |
| X4 | **One grammar.** Subcommands are verbs (`serve`, `search`, `reindex`, `schedule`, `graph`, `status`, `verify`); flags are kebab-case (`-copy-first`, `-enable-offline`, `-sync-wait`, `-client-secret-file`); repeated concepts share a name (`-out`, `-input`, `-mode`, `-log`). A *flag* (`-outlook`) augments the default export; a *subcommand* (`graph`) is a distinct acquisition mode; `schedule … -- <job>` carries any backup job verbatim. | Met (by convention) | Reviewed; consistent. **C4:** a parser-tree lint would make this structural (backlog). |
| X5 | **Flags mean the same everywhere.** `-out` is the archive dir across export/serve/search/reindex/schedule/graph/status/verify; `-input` is always a source; `-mode` is always incremental/full; `-log` is always the run log file. The scheduler dry-parses jobs through the same flag builders the jobs run with (`cmd/mailarchive/job.go`), so a flag cannot mean two things. | Met | Same flag names carry the same meaning across every surface; MA-72 proves the shared parse. |
| X6 | **A legibility surface per feature.** Every feature's state is inspectable with GREEN/WARN/RED posture, each WARN/RED naming its remedy. | Met | `mailarchive status -out DIR` (MA-75, R18): completeness (fillable/terminal/unknown), fixity coverage, last run (running/ok/failed/cancelled, MA-76), schedule (descriptor + three-state host query, MA-78), executable identity; fails closed on missing evidence, including a present-but-unreadable manifest (RED, MA-159). `verify`'s integrity verdict is recorded separately and surfaces here as a `Last verify` line that drives the posture RED when the last verify found modified or missing files, and WARN when it only left files unrecorded or is stale against its own verify schedule (MA-154, MA-155); `status -json` carries a machine-readable `reason_codes` array parallel to the reasons. The export summary and the GUI summary/health view carry the same counts. |
